...
Notice: Some function names have been changed relative to version 1.04. 
If your current version is not 1.05 or later, an update is recommended.
...
\version 02-06-2012 1.05 Some function names change. Added documentation.
...
public:
    ...
    /*! Sets the camera field of view to use.
        \param f New field of view in degrees */
    void SetFieldOfView(double f) {m_fieldofview = f;}

    //! Get the set field of view
    //! \return Field of view in degrees
    double GetFieldOfView() const {return m_fieldofview;}

    //! Get the camera eye
    //! \return Camera eye as a 4D vector with w=1
    const double *GetEye() const {return m_eye;}

    //! Get the camera center
    //! \return Camera center as a 4D vector with w=1
    const double *GetCenter() const {return m_center;}

    //! Get the camera UP direction
    //! \return Camera up direction as a 4D vector with w=0
    const double *GetUp() const {return m_up;}

    //! Get camera Eye location X component
    //! \return Camera eye location X component
    double GetEyeX() const {return m_eye[0];}

    //! Get camera Eye location Y component
    //! \return Camera eye location Y component
    double GetEyeY() const {return m_eye[1];}

    //! Get camera Eye location Z component
    //! \return Camera eye location Z component
    double GetEyeZ() const {return m_eye[2];}

    //! Get camera center location X component
    //! \return Camera center location X component
    double GetCenterX() const {return m_center[0];}

    //! Get camera center location Y component
    //! \return Camera center location Y component
    double GetCenterY() const {return m_center[1];}

    //! Get camera center location Z component
    //! \return Camera center location Z component
    double GetCenterZ() const {return m_center[2];}

    //! Get camera up direction X component
    //! \return Camera up direction X component
    double GetUpX() const {return m_up[0];}

    //! Get camera up direction Y component
    //! \return Camera up direction Y component
    double GetUpY() const {return m_up[1];}

    //! Get camera up direction Z component
    //! \return Camera up direction Z component
    double GetUpZ() const {return m_up[2];}

    //! Get a computed ZNear distance
    /*! This function computes an appropriate ZNear distances 
        based on the distance from the camera to the center multiplied by
        the value set by SetZNearRange(). 
        \return Computed ZNear value (positive). */
    double GetZNear() const {return CameraDistance() * m_zNearRange;}

    //! Get a computed ZFar distance
    /*! This function computes an appropriate ZFar distances 
        based on the distance from the camera to the center multiplied by
        the value set by SetZFarRange(). 
        \return Computed ZFar value (positive). */
    double GetZFar() const {return CameraDistance() * m_zFarRange;}
