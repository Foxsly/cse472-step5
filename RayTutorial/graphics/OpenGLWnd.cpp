...
int COpenGLWnd::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
    ...
    if (nPixelFormat == 0)
        return FormattedErrorAfxMsgBox(TEXT("ChoosePixelFormat Failed"));

    BOOL bResult = SetPixelFormat (dc.m_hDC, nPixelFormat, &pfd);
    if(!bResult)
        return FormattedErrorAfxMsgBox(TEXT("SetPixelFormat Failed"));
    ...
    if (m_hrc == 0)
        return FormattedErrorAfxMsgBox(TEXT("Unable to create an OpenGL rendering context"));
...
void COpenGLWnd::OnPaint() 
{
    ...
    if (!bResult)
    {
        FormattedErrorAfxMsgBox(TEXT("wglMakeCurrent Failed"));
    }
...
int COpenGLWnd::FormattedErrorAfxMsgBox(const TCHAR *p_msg)
{
    LPVOID lpMsgBuf;
    FormatMessage( 
...
#ifdef UNICODE
    wstringstream str;
    str << p_msg << L" " << lpMsgBuf;
    AfxMessageBox(str.str().c_str(), MB_OK | MB_ICONSTOP);
#else
    stringstream str;
    str << p_msg << " " << lpMsgBuf;
    AfxMessageBox(str.str().c_str(), MB_OK | MB_ICONSTOP);
